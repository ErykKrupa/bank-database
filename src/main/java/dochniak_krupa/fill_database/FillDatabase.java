  ...
  private static void fillClientsWithEncryptedPasswordsAndBDAccounts(int number) {
    for (int i = 0; i < number; i++) {
      try (Session session = HibernateUtility.getSessionFactory().openSession()) {
        ...
        session.createSQLQuery("GRANT SELECT ON bank.debit_card TO `" + login + "`@`localhost`;").executeUpdate();
        session.createSQLQuery("GRANT SELECT ON bank.credit_card TO `" + login + "`@`localhost`;").executeUpdate();;
